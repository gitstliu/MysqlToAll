...
func (c *Client) DoRequest(method string, url string, body *bytes.Buffer) (*http.Response, error) {
	...
	log4go.Debug("req Header", req.Header)
	resp, err := c.c.Do(req)
...
func (c *Client) DoBulk(url string, items []*BulkRequest) (*BulkResponse, error) {
	...
	log4go.Debug(string(buf.Bytes()))

	resp, err := c.DoRequest("POST", url+"/_bulk", &buf)
	log4go.Debug("Do es request finished")
	log4go.Debug("Do es request err %v", err)
	if err != nil {
		log4go.Error(err)
		return nil, errors.Trace(err)
	}
	...
	data, err := ioutil.ReadAll(resp.Body)
	log4go.Debug("Read response err %v", err)
	if err != nil {
		log4go.Error(err)
		return nil, errors.Trace(err)
	}
	...
	if len(data) > 0 {
		log4go.Debug("data %v", string(data))
		err = json.Unmarshal(data, &ret)
		log4go.Debug("Read response Unmarshal err %v", err)
	}

	log4go.Debug("DoBulk finished")
	return ret, errors.Trace(err)
}
